import { $ as fcumsum, A as projection, B as extent_default, C as azimuthalEquidistant_default, D as albers_default, E as albersUsa_default, F as graticule, G as rotation_default, H as circle_default$1, I as graticule10, J as area_default$1, K as centroid_default, L as contains_default, M as transform_default, N as path_default, O as conicEqualAreaRaw, P as interpolate_default, Q as Adder, R as distance_default, S as azimuthalEquidistantRaw, T as azimuthalEqualArea_default, U as antimeridian_default, V as clipRectangle, W as circle_default, X as range, Y as stream_default, Z as merge, _ as equirectangular_default, a as orthographicRaw, b as mercatorRaw, c as naturalEarth1_default, d as gnomonic_default, et as fsum, f as equalEarthRaw, g as equirectangularRaw, h as conicEquidistant_default, i as stereographic_default, j as projectionMutator, k as conicEqualArea_default, l as identity_default, m as conicEquidistantRaw, n as transverseMercator_default, o as orthographic_default, p as equalEarth_default, q as bounds_default, r as stereographicRaw, s as naturalEarth1Raw, t as transverseMercatorRaw, u as gnomonicRaw, v as conicConformalRaw, w as azimuthalEqualAreaRaw, x as mercator_default, y as conicConformal_default, z as length_default } from "./src-BeqOcuNl.js";
...
function axis(orient, scale) {
	...
	function axis(context) {
		var values = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain() : tickValues, format = tickFormat == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : identity_default$3 : tickFormat, spacing = Math.max(tickSizeInner, 0) + tickPadding, range = scale.range(), range0 = +range[0] + offset, range1 = +range[range.length - 1] + offset, position = (scale.bandwidth ? center : number$2)(scale.copy(), offset), selection = context.selection ? context.selection() : context, path = selection.selectAll(".domain").data([null]), tick = selection.selectAll(".tick").data(values, scale).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
...
function hue(a, b) {
	var d = b - a;
	return d ? linear$2(a, d > 180 || d < -180 ? d - 360 * Math.round(d / 360) : d) : constant_default$7(isNaN(a) ? b : a);
}
function gamma(y) {
	return (y = +y) === 1 ? nogamma : function(a, b) {
		return b - a ? exponential(a, b, y) : constant_default$7(isNaN(a) ? b : a);
	};
}
function nogamma(a, b) {
	var d = b - a;
	return d ? linear$2(a, d) : constant_default$7(isNaN(a) ? b : a);
}
//#endregion
...
function value_default(a, b) {
	var t = typeof b, c;
	return b == null || t === "boolean" ? constant_default$7(b) : (t === "number" ? number_default : t === "string" ? (c = color(b)) ? (b = c, rgb_default) : string_default : b instanceof color ? rgb_default : b instanceof Date ? date_default : isNumberArray(b) ? numberArray_default : Array.isArray(b) ? genericArray : typeof b.valueOf !== "function" && typeof b.toString !== "function" || isNaN(b) ? object_default : number_default)(a, b);
}
//#endregion
...
var pi$3 = Math.PI, halfPi$2 = pi$3 / 2;
function sinIn(t) {
...
var tau$4 = 2 * Math.PI, amplitude = 1, period = .3;
...
var { abs: abs$2, max: max$3, min: min$2 } = Math;
function number1(e) {
	return [+e[0], +e[1]];
}
function number2(e) {
...
var pi$1 = Math.PI, tau$2 = 2 * pi$1, epsilon$3 = 1e-6, tauEpsilon = tau$2 - epsilon$3;
...
var Path = class {
	...
	arcTo(x1, y1, x2, y2, r) {
		...
		if (this._x1 === null) this._append`M${this._x1 = x1},${this._y1 = y1}`;
		...
		else if (!(Math.abs(y01 * x21 - y21 * x01) > epsilon$3) || !r) this._append`L${this._x1 = x1},${this._y1 = y1}`;
		else {
			...
			if (Math.abs(t01 - 1) > epsilon$3) this._append`L${x1 + t01 * x01},${y1 + t01 * y01}`;
	...
	arc(x, y, r, a0, a1, ccw) {
		...
		else if (Math.abs(this._x1 - x0) > epsilon$3 || Math.abs(this._y1 - y0) > epsilon$3) this._append`L${x0},${y0}`;
		if (!r) return;
		if (da < 0) da = da % tau$2 + tau$2;
		if (da > tauEpsilon) this._append`A${r},${r},0,1,${cw},${x - dx},${y - dy}A${r},${r},0,1,${cw},${this._x1 = x0},${this._y1 = y0}`;
		else if (da > epsilon$3) this._append`A${r},${r},0,${+(da >= pi$1)},${cw},${this._x1 = x + r * Math.cos(a1)},${this._y1 = y + r * Math.sin(a1)}`;
	}
	rect(x, y, w, h) {
...
var tau$1 = 2 * Math.PI, pow$1 = Math.pow;
function pointX(p) {
	return p[0];
}
function pointY(p) {
...
function locale_default(locale) {
	var group = locale.grouping === void 0 || locale.thousands === void 0 ? identity_default$2 : formatGroup_default(map$1.call(locale.grouping, Number), locale.thousands + ""), currencyPrefix = locale.currency === void 0 ? "" : locale.currency[0] + "", currencySuffix = locale.currency === void 0 ? "" : locale.currency[1] + "", decimal = locale.decimal === void 0 ? "." : locale.decimal + "", numerals = locale.numerals === void 0 ? identity_default$2 : formatNumerals_default(map$1.call(locale.numerals, String)), percent = locale.percent === void 0 ? "%" : locale.percent + "", minus = locale.minus === void 0 ? "−" : locale.minus + "", nan = locale.nan === void 0 ? "NaN" : locale.nan + "";
